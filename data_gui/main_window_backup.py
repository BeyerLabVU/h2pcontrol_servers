...
class MainWindow(QMainWindow):
    def __init__(self, loop=None):
        ...
        self.active_stream_node_id = None        self.setWindowTitle("Oscilloscope: Direct Trace View")
        ...
        # ---- Data Sources ----
        self.data_sources = {}
        available_stream_names = list(self.data_sources.keys())        # ---- Control panel (scroll area on the right) ----
    ...
    def configure_test_catchment(self) -> None:
        ...
        if not self.test_catchment_initialized:            
            ...
            self.add_test_catchment_action.setEnabled(False)  # Disable after adding    @Slot(str)
    def on_stream_selected(self, stream_name: str):
        print(f"MainWindow: Stream selected: {stream_name}")        # Unsubscribe all PlotNodes from previous input
    ...
    def start_selected_data_stream(self, stream_name: str):
        ...
        if data_source:
            ...
            self.plot_status_bar.update_status(f"Running: {stream_name}")        else:
    ...
    def open_add_node_dialog(self):
        ...
        if dlg.exec_() == QDialog.Accepted: # type: ignore
            ...
            # # Check to see if the new node should be subscribed to something
            # if new_node.input_nodes:
